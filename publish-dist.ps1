# Compila Modulo.Rendiciones y deja dist/Modulo.Rendiciones/{version}/ limpio -- ese
# folder es el DESTINO de una junction real (ver `fsutil reparsepoint query` sobre
# "Portal SaaS - Core\artifacts\plugins\Modulo.Rendiciones") hacia
# "Portal SaaS - Core\artifacts\plugins\Modulo.Rendiciones", así que limpiar acá alcanza
...
# (EF Core SqlServer/Npgsql, Azure.AI.DocumentIntelligence) con
...
# AssemblyDependencyResolver, que LEE Modulo.Rendiciones.deps.json. Por eso, a
# diferencia del script de Core, ACÁ *.deps.json NUNCA se borra -- borrarlo rompe la
# conexión a SQL Server/Postgres en el primer uso real.
#
...
if (-not (Test-Path $depsJson)) {
    throw "Falta $depsJson -- SIN esto, PluginLoadContext no puede resolver las dependencias NuGet propias del módulo (EF Core SqlServer/Npgsql, Azure SDK) en runtime"
}
