-- Permite que Reglas_Distribucion.NroCuenta sea un patrón SQL LIKE (ej. '61-09-%') en vez de exigir
-- coincidencia exacta -- así una sola regla puede cubrir una familia de cuentas, en vez de tener que
-- cargar una fila por cada cuenta a mano. Retrocompatible: toda regla existente sin '%' sigue
-- matcheando exactamente igual que antes, porque LIKE sin comodín se comporta igual que "=".
-- Único cambio real respecto a la versión anterior (Sql/sp_EjecutarDistribucionAutomatica_CascadaSucursal.sql):
-- el JOIN de LineasConRegla pasa de "r.NroCuenta = d.NroCuenta" a "d.NroCuenta LIKE r.NroCuenta".
...
BEGIN
    ...
    BEGIN TRY
        ...
        --    (match por Cuenta -- ahora vía LIKE, admite patrones -- + CC, o por Cuenta si la regla no especifica CC)
        ...
        SELECT
            ...
            d.MontoOriginal,
            -- Centro de costo: si se distribuye por SUCURSAL y esa sucursal está en el maestro, se cascadea; si no, igual al original.
            CASE WHEN v.TipoBase = 'SUCURSAL' THEN ISNULL(msuc.CodCentroCosto, d.CodCentroCostoOriginal) ELSE d.CodCentroCostoOriginal END,
            CASE WHEN v.TipoBase = 'SUCURSAL' THEN ISNULL(msuc.CentroCosto, d.CentroCostoOriginal) ELSE d.CentroCostoOriginal END,
            -- Canal: si la base es CANAL, viene directo de la vista; si es SUCURSAL, se cascadea desde el maestro (o queda igual si no está mapeada).
            CASE
                WHEN v.TipoBase = 'CANAL' THEN v.CodDimension
                WHEN v.TipoBase = 'SUCURSAL' THEN ISNULL(msuc.CodCanal, d.CodCanalOriginal)
                ELSE d.CodCanalOriginal
            END,
            CASE
                WHEN v.TipoBase = 'CANAL' THEN v.NombreDimension
                WHEN v.TipoBase = 'SUCURSAL' THEN ISNULL(msuc.Canal, d.CanalOriginal)
                ELSE d.CanalOriginal
            END,
            CASE WHEN v.TipoBase = 'SUCURSAL' THEN v.CodDimension ELSE d.CodSucursalOriginal END,
            CASE WHEN v.TipoBase = 'SUCURSAL' THEN v.NombreDimension ELSE d.SucursalOriginal END,
        ...
        LEFT JOIN dbo.Maestro_Sucursal msuc
            ON msuc.CodSucursal = v.CodDimension AND msuc.Activo = 1 AND v.TipoBase = 'SUCURSAL';
