# Compila Modulo.GestionDistribucionGastos y deja
# dist/Modulo.GestionDistribucionGastos/{version}/ limpio -- ese folder es el DESTINO
# de una junction real (ver `fsutil reparsepoint query` sobre
# "Portal SaaS - Core\artifacts\plugins\Modulo.GestionDistribucionGastos"), así que
# limpiar acá alcanza -- no hace falta copiar nada al portal.
#
...
# (EF Core SqlServer, ClosedXML) con CopyLocalLockFileAssemblies=true.
# PluginLoadContext (Core/Infraestructura/) resuelve esas dependencias -- managed Y
# nativas (Microsoft.Data.SqlClient.SNI.dll) -- vía AssemblyDependencyResolver, que LEE
# Modulo.GestionDistribucionGastos.deps.json. Por eso, a diferencia del script de Core,
# ACÁ *.deps.json NUNCA se borra -- borrarlo rompe la conexión a la base SQL Server
# externa (CLDEPORFIN) en el primer uso real.
#
...
$root = $PSScriptRoot
$moduleName = "Modulo.GestionDistribucionGastos"
...
if (-not (Test-Path $depsJson)) {
    throw "Falta $depsJson -- SIN esto, PluginLoadContext no puede resolver las dependencias NuGet propias del módulo (EF Core SqlServer, ClosedXML) en runtime"
}
