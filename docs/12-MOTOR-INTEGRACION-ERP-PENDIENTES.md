...
Investigado y diferido explícitamente en la ronda de Fase 2. Bloqueante real: `Modulo.Wms` no tiene todavía ninguna tabla de staging con datos reales pendientes de enviar a SAP (`wms_oracle_inbound_stage` y afines existen solo en `ARQUITECTURA.md` de Wms, no en código). La entidad candidata mejor definida es la confirmación de traslado (`ORDER_CONFIRM_STOCKTRANSFER`, mapea 1:1 a `InventoryDocumentService.CreateAsync`). Antes de implementar `IIntegrationEntityReader` real para Wms hay que construir esa tabla de staging — es un desarrollo propio, no solo "conectar una interfaz". Este punto es el que además destrabaría el punto 1 (primer caso real de mapeo DTO).
...
- `WmsSapIntegration.Service` (Windows Service externo, captura por trigger HANA) — no se toca ni se reemplaza; resuelve un problema distinto (captura de eventos, no orquestación).
