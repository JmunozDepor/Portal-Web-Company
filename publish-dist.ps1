# Compila Modulo.Wms y deja dist/Modulo.Wms/{version}/ limpio -- ese folder es el
# DESTINO de una junction real (ver `fsutil reparsepoint query` sobre
# "Portal SaaS - Core\artifacts\plugins\Modulo.Wms") hacia
# "Portal SaaS - Core\artifacts\plugins\Modulo.Wms", así que limpiar acá alcanza --
# no hace falta copiar nada al portal. Portado tal cual de
# Modulo.Rendiciones/publish-dist.ps1 (mismo criterio, ver ARQUITECTURA.md
# "Falta explícitamente" -- este script no existía, la copia se hacía a mano).
#
...
# (EF Core SqlServer/Npgsql) con CopyLocalLockFileAssemblies=true. PluginLoadContext
# (Core/Infraestructura/) resuelve esas dependencias -- managed Y nativas
# (Microsoft.Data.SqlClient.SNI.dll) -- vía AssemblyDependencyResolver, que LEE
# Modulo.Wms.deps.json. Por eso, a diferencia del script de Core, ACÁ *.deps.json
# NUNCA se borra -- borrarlo rompe la conexión a SQL Server/Postgres en el primer
# uso real.
#
...
Get-ChildItem -Path $versionDir -Recurse -File | ForEach-Object {
    ...
    # runtimes\{rid}\... -- solo se conservan los RIDs de la whitelist (win, win-x64);
    # el resto (unix, win-arm, win-arm64, win-x86) nunca aplica en este servidor.
...
# Limpiar carpetas de RID/culture que quedaron vacías tras borrar sus archivos.
