-- Extiende la cascada Sucursal->Canal->Centro de costo (que hoy solo mira TipoBase='SUCURSAL')
-- para que también aplique a la nueva base 'SUCURSAL_TPR_ECM' -- es la misma lógica de reparto por
-- sucursal, solo que calculada sobre un subconjunto de sucursales (ver
-- Sql/vw_VentaBaseDistribucion_SucursalTprEcm.sql). Único cambio respecto a la versión anterior
-- (Sql/sp_EjecutarDistribucionAutomatica_PatronCuenta.sql): cada "v.TipoBase = 'SUCURSAL'" pasa a
-- "v.TipoBase IN ('SUCURSAL', 'SUCURSAL_TPR_ECM')".
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            d.MontoOriginal,
            -- Centro de costo: si se distribuye por sucursal (SUCURSAL o SUCURSAL_TPR_ECM) y esa
            -- sucursal está en el maestro, se cascadea; si no, igual al original.
            CASE WHEN v.TipoBase IN ('SUCURSAL', 'SUCURSAL_TPR_ECM') THEN ISNULL(msuc.CodCentroCosto, d.CodCentroCostoOriginal) ELSE d.CodCentroCostoOriginal END,
            CASE WHEN v.TipoBase IN ('SUCURSAL', 'SUCURSAL_TPR_ECM') THEN ISNULL(msuc.CentroCosto, d.CentroCostoOriginal) ELSE d.CentroCostoOriginal END,
            ...
            CASE
                ...
                WHEN v.TipoBase IN ('SUCURSAL', 'SUCURSAL_TPR_ECM') THEN ISNULL(msuc.CodCanal, d.CodCanalOriginal)
                ELSE d.CodCanalOriginal
            END,
            CASE
                WHEN v.TipoBase = 'CANAL' THEN v.NombreDimension
                WHEN v.TipoBase IN ('SUCURSAL', 'SUCURSAL_TPR_ECM') THEN ISNULL(msuc.Canal, d.CanalOriginal)
                ELSE d.CanalOriginal
            END,
            CASE WHEN v.TipoBase IN ('SUCURSAL', 'SUCURSAL_TPR_ECM') THEN v.CodDimension ELSE d.CodSucursalOriginal END,
            CASE WHEN v.TipoBase IN ('SUCURSAL', 'SUCURSAL_TPR_ECM') THEN v.NombreDimension ELSE d.SucursalOriginal END,
        ...
        LEFT JOIN dbo.Maestro_Sucursal msuc
            ON msuc.CodSucursal = v.CodDimension AND msuc.Activo = 1 AND v.TipoBase IN ('SUCURSAL', 'SUCURSAL_TPR_ECM');
