...
public sealed class WmsSvshStageParser : BackgroundService
{
    ...
    private async Task RegistrarHeartbeatAsync(Guid companyId, string estado, string? mensaje, CancellationToken cancellationToken)
    {
        try
        {
            ...
            await heartbeat.RecordAsync(companyId, "Wms.SvshStageParser", estado, mensaje, cancellationToken);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error registrando el heartbeat '{Estado}' del SvshStageParser", estado);
        }
    }
    ...
    private async Task RegistrarFalloDePersistenciaAsync(Guid companyId, long entryId, Exception fallo, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var maxIntentos = await recoveryScope.ServiceProvider
                ...
                .GetIntAsync(WmsRuntimeSettingsKeys.SvshParserMaxIntentos, MaxIntentosDefault, cancellationToken);
