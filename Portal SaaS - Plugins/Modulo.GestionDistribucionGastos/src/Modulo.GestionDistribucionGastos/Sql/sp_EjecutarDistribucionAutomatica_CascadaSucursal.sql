-- Agrega la cascada Sucursal -> Canal -> Centro de costo (vía Maestro_Sucursal) al motor automático.
-- Antes: al distribuir por BaseDistribucion='SUCURSAL', el SP solo corregía CodSucursalDestino;
-- Canal y Centro de costo quedaban con el valor original de la línea (potencialmente el mal digitado).
-- Ahora: si la sucursal asignada está en Maestro_Sucursal (y Activo=1), su Canal y Centro de costo
-- se usan como destino también. Si no está en el maestro, cae al valor original (mismo comportamiento
-- que antes). La distribución por BaseDistribucion='CANAL' no cambia: sigue tocando solo Canal.
...
BEGIN
    ...
    BEGIN TRY
        ...
        --    (match por Cuenta + CC, o por Cuenta si la regla no especifica CC)
